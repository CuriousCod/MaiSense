	...
	private:
		...
		json LoadConfig(std::string filename) {
			...
			try {
				...
				MessageBoxA(NULL, "Could not parse sensor.config. Please verify that the json is valid.", "Config Error", MB_ICONEXCLAMATION);
				return NULL; 
			}
			...
			if (configExists){
				...
				for (std::string key : configKeys) {
					if (!config.contains(key)){
						MessageBoxA(NULL, ("Could not find key: " + key + " from sensor.config. Skipping config.").c_str(), "Config Error", MB_ICONEXCLAMATION);
						configExists = false;
					}
				}
